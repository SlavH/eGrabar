...
export default function AdminAmarasPage() {
  ...
  async function fetchAmaras() {
    ...
    const { data } = await supabase.from('amaras').select('*');
  ...
  async function handleDelete(id: string) {
    if (!confirm(language === 'en' ? 'Are you sure?' : 'Վստա՞հ եք:')) return;
    const { supabase } = await import('@/lib/supabase');
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-white">{language === 'en' ? 'Amaras Content Management' : 'Ամարասի բովանդակության կառավարում'}</h1>
        <button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ title_en: '', title_hy: '', content_en: '', content_hy: '', show_on_home: false }); }} className="px-4 py-2 bg-gradient-to-br from-blue-600 to-blue-800 text-white font-semibold rounded-lg">
          {showForm ? (language === 'en' ? 'Cancel' : 'Չեղարկել') : (language === 'en' ? 'Add New Section' : 'Ավելացնել')}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 mb-8">
          ...
          <button className="px-6 py-2 bg-blue-600/80 backdrop-blur-md text-white font-semibold rounded-lg mt-4 hover:bg-blue-600 transition-colors">{language === 'en' ? 'Save' : 'Պահպանել'}</button>
        </form>
      )}
            ...
            {amaras.map(item => (
              <tr key={item.id} className="border-b border-white/5">
                ...
                <td className="p-4">
                  ...
                  <button onClick={() => handleDelete(item.id)} className="text-red-300 hover:text-red-200 text-sm">{language === 'en' ? 'Delete' : 'Ջնջել'}</button>
                </td>
              </tr>
            ))}
          </tbody>
